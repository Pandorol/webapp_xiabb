...
<jsp:include page="userinfohead.jsp"></jsp:include>
		 ...
		 <%
			...
			List<Recommendpostinfo> myposts = (List<Recommendpostinfo>)session.getAttribute("myposts");
		 if(myposts.size()>0){
			int pagesize=4;
			int pagecount ;
			pagecount = myposts.size()%pagesize==0 ? myposts.size()/pagesize : myposts.size()/pagesize+1;
			int pagenow=1;
	...
	%>
	<%for(int i=0;i<pagesize&&i+(pagenow-1)*4<myposts.size();i++){ 
		Recommendpostinfo p = myposts.get((pagenow-1)*4+i);
	...
	<%} %>
		<div style="text-align:center;font-size:1.5em;margin-bottom:20px;"><a href="myposts.jsp?pagenow=1">首页</a>|<a href="myposts.jsp?pagenow=<%=pagenow-1 %>">上一页</a>|<a href="myposts.jsp?pagenow=<%=pagenow+1 %>">下一页</a>|<a href="myposts.jsp?pagenow=<%=pagecount %>">尾页</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <%=pagenow %>/<%=pagecount %></div>
